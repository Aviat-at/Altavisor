...
function Section({ title, description, children }: any) {
  return (
    <Paper sx={{ mb: 2.5, bgcolor: "#13151A", border: "1px solid", borderColor: "divider", borderRadius: 2, overflow: "hidden" }}>
    ...
    <Box>
      <Section title="Onboarding" description="Defaults applied when a new customer is created">
        <FieldRow label="Default Plan" hint="Plan assigned on signup">
          <FormControl size="small" sx={{ minWidth: 160 }}>
            <Select value={defaultPlan} onChange={(e) => setDefaultPlan(e.target.value)} sx={{ bgcolor: "rgba(255,255,255,0.03)" }}>
        ...
        <FieldRow label="Events">
          <Box sx={{ display: "flex", gap: 0.8, flexWrap: "wrap" }}>
            {["customer.created", "customer.updated", "customer.churned", "plan.changed"].map((ev) => (
              <Chip key={ev} label={ev} size="small" sx={{ fontFamily: "monospace", fontSize: "0.68rem", bgcolor: "rgba(255,255,255,0.05)", color: "text.secondary" }} />
            ))}
          </Box>
        </FieldRow>
      </Section>
      ...
      <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
        <Button variant="contained" startIcon={<SaveRoundedIcon />}
          ...
          sx={{ bgcolor: "#C8F04A", color: "#0D0E10", fontWeight: 700, "&:hover": { bgcolor: "#D4F55A" } }}>
